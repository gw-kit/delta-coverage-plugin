...
internal fun analyzableReportFactory(
    ...
    allReports += DeltaCoverageAnalyzableReport(
        reportContext,
        ReportBound.DELTA_REPORT.buildJacocoReports(reportsConfig, enabledReportTypes),
    )

    if (reportsConfig.fullCoverageReport) {
        allReports += FullCoverageAnalyzableReport(
            ReportBound.FULL_REPORT.buildJacocoReports(reportsConfig, enabledReportTypes)
        )
    }
